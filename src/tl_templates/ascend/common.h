...
// gemm_v0 with the per-N-tile fixpipe fused in: each [M, nTile] tile is
// fixpipe'd straight to GM as soon as its K accumulation finishes, so L0C only
// ever holds one [M, nTile] tile. Unlike gemm_v0 -- which keeps the whole
// [M, N] result resident in L0C until the caller copies it out (a large N,
// e.g. N=512, is 512*128*2 = 128KB = the entire L0C) -- this writes each
// N-tile straight to the GM destination, so the L0C accumulator C is a single
// [M, nTile] slot (e.g. [64,128]=32KB). Same N/K tiling and L0A/L0B ping-pong
// as gemm_v0; the only addition is the per-tile copy_l0c_to_gm and the
// M_FIX/FIX_M handshake that lets the single L0C slot be reused per tile.
// (Single K tile here: K <= 128 fits one kL0 tile, so no K-splitting.)
...
gemm_v0_fixp(LocalTensor<T1> const &A, LocalTensor<T1> const &B,
  ...
  // rows of the K dim are loaded into L0 and contracted -- contracting over the
  // actual valid length rather than a padded tile. This keeps the matmul from
  // summing unwritten tail rows of the input (e.g. an underfilled tile whose
  // pad rows are uninitialised L1 on first use): summing 0 (masked) * NaN
  // (garbage) -> NaN would otherwise poison the result. Single K tile only
  // (K <= kL0Size); every gemm_v0_fixp caller satisfies it.
